...
output "db_proxy_endpoint_name" {
  description = "The connection endpoint"
  value       = aws_db_proxy_endpoint.rds-proxy.endpoint
}

output "aws_db_snapshot-jobsearchservice" {
  value = data.aws_ebs_snapshot.jobsearchservice.id
}

output "aws_db_snapshot-dev-Zulip" {
  value = data.aws_ebs_snapshot.dev-zulip-snap.id
}
